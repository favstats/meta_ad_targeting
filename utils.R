...
get_page_insights <- function(pageid, timeframe = "LAST_30_DAYS", lang = "en-GB", iso2c = "US", include_info = c("page_info", "targeting_info"), join_info = T) {
  ...
  if(!is.null(out[[1]][["errors"]][["description"]])){
    stop(paste0("Rate limit exceeded: ", out[[1]][["errors"]][["description"]]))
  }
  ...
  if( "page_info" %in% include_info) {
    ...
    if (!is.null(creat_times_raw)) {
      creat_times <- creat_times_raw %>%
      ...
      creat_times <- tibble(no_events  = T)
    }
    
      
    about_text_raw <- out[[1]][["data"]][["page"]][["about"]] 
    if(!is.null(about_text_raw)){
      about_text <- about_text_raw %>%  purrr::set_names("about")
    } else {
